...
def validate(doc,method):
	...
	if doc.asset and len(doc.get("last_readings"))<=3 and doc.status == 'Open':
		...
		for d in range(len(mr_all)-1):
			if mr_all[d]['total'] != '0' and mr_all[d]['total'] is not None and mr_all[d+1]['total'] is not None:
...
def validate_current_reading(doc):
	if frappe.db.get_value('Type of Call',{'name':doc.type_of_call},'ignore_reading')==0 and len(doc.get("current_reading"))==0 or (frappe.db.get_value('Asset Readings',{'parent':doc.name},'reading') == None and frappe.db.get_value('Asset Readings',{'parent':doc.name},'reading_2') == None) and doc.type_of_call != 'Toner':
